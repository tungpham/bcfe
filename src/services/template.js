...
export default {
	...
	// new apis
	createRoot: (name, type, value, desc) => xapi().post(TEMPL_API_PATH + 'nodes', {
		name, type, value, description: desc
	}).then(res => res.data),
	createNode: (parent, name, type, value, desc) => xapi().post(TEMPL_API_PATH + 'nodes/' + parent, {
		name, type, value, description: desc
	}).then(res => res.data),
	getNode: id => xapi().get(TEMPL_API_PATH + 'nodes/' + id).then(res => res.data),
	updateNode: (id, name, type, value, desc) => xapi().put(TEMPL_API_PATH + 'nodes/' + id, {
		name, type, value, description: desc
	}),
